...
defmodule EdgehogDeviceForwarderWeb.UserController do
  ...
  def handle_in(conn, params) do
    with {:ok, session} <- SessionCookie.fetch(conn) do
      protocol = session.protocol |> String.downcase(:ascii)

      case protocol do
        "http" -> handle_http(conn, params, session)
        "https" -> handle_http(conn, params, Map.put(session, :secure, true))
        other -> {:error, {:invalid_protocol, other}}
      end
    end
  end

  @spec handle_http(Plug.Conn.t(), any, SessionCookie.session()) ::
          Plug.Conn.t()
          | {:error, :invalid_request_port}
          | {:error, :request_timeout}
          | {:error, :token_not_found}
  defp handle_http(conn, _params, session) do
    with {:ok, port} <- fetch_port(session.port),
         {:ok, request} <- build_http_request(conn, port) do
      case Forwarder.http_to_device(session.session, request, http_opts(session)) do
        {:respond, response} ->
          conn
  ...
  @spec build_http_request(Plug.Conn.t(), integer) :: {:ok, HTTP.Request.t()}
  ...
  @spec http_opts(SessionCookie.session()) :: keyword
  defp http_opts(session) do
    case Map.fetch(session, :secure) do
      {:ok, true} -> [secure: true]
      _ -> []
    end
  end
